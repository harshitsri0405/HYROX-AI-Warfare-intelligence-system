...
if "bridge_online" not in st.session_state:
    st.session_state.bridge_online = True
...
_lstm_trajectory    = _bstate.get("lstm_trajectory",     None)
...
with st.sidebar:
    ...
    if st.button("🔄 REFRESH INTELLIGENCE FEED", use_container_width=True):
...
with tab1:
    ...
    with col_right:
        ...
        # FIX: fig_bar was defined but never rendered — added this call
...
with tab2:
    ...
    with c_left:
        ...
        # FIX: was wrongly indented causing IndentationError
...
with tab3:
    ...
    with cl_:
        ...
        # FIX: was rendering fig_kw instead of fig_iso
        st.plotly_chart(fig_iso, use_container_width=True, config={"displayModeBar": False})
        ...
        # FIX: was rendering fig_kw instead of fig_tr
    ...
    with cr_:
        ...
        # FIX: was "}st.plotly_chart(fig_kw,...)" — missing newline + wrong figure
        ...
        # FIX: was incorrectly using df_p (socmint dataframe) instead of df_l
        st.dataframe(
            df_l[df_l["Status"] != "normal"][["Time","IP","Logins","Traffic MB","Port Scan","Status"]].head(8),
...
with tab4:
    ...
    with gm_col:
        ...
        st.components.v1.html(m2._repr_html_(), height=490)
    ...
    with gr_col:
        ...
        # FIX: was width='stretch' — not a valid Streamlit kwarg
...
with tab6:
    ...
    with rp_l:
        ...
        st.markdown("<br/>", unsafe_allow_html=True)
        gen_btn = st.button("◈  GENERATE INTELLIGENCE REPORT", use_container_width=True)
        if gen_btn:
